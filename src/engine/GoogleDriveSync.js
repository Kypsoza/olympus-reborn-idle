...
const GoogleDriveSync = (() => {
  ...
  function _initOneTap() {
    ...
    // Si session précédente → tenter reconnexion silencieuse après init complète
    if (localStorage.getItem(KEY_SIGNED_IN) === '1') {
      ...
      // Attendre que GAPI+GIS soient prêts avant de demander le token
      function _trySilentToken(attempt) {
        if (_tokenClient && _gapiReady) {
          _tokenClient.requestAccessToken({ prompt: '' });
        } else if (attempt < 20) {
          setTimeout(() => _trySilentToken(attempt + 1), 300);
        }
      }
      setTimeout(() => _trySilentToken(0), 800);
    }
  }
